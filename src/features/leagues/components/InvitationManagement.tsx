...
export function InvitationManagement({ leagueId, opened, onClose }: InvitationManagementProps) {
  ...
  const handleCsvUpload = async (participants: Array<{ email: string; name?: string; class?: string }>) => {
    if (!user?.id) {
      notifications.show({
        color: 'red',
        title: 'Ошибка',
    ...
    >
      <Stack gap="lg">
        ...
        <Group gap="xs">
          ...
          <Button
            variant={activeTab === 'csv' ? 'filled' : 'light'}
        ...
        {activeTab === 'csv' && (
          <CsvUploader
            ...
            isLoading={bulkAddParticipants.isPending}
          />
        )}
